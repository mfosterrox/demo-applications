...
**Podman (local, this repo)**  
Same [docker/genai-stack](https://github.com/docker/genai-stack) layout; use the merge file `compose.podman.yaml` so `host.docker.internal` reaches the host (Ollama on `ollama serve`, port 11434). From the **demo-applications** repo root: `make genai-podman-up` (uses `podman compose -f docker-compose.yml -f compose.podman.yaml up --build -d`). Stop with `make genai-podman-down`; follow logs with `make genai-podman-logs`. Copy `env.example` to `.env` in this directory first; see `env.podman.local.example` for optional overrides. For Ollama inside containers (Linux-style), use `--profile linux` and set `OLLAMA_BASE_URL=http://llm:11434` in `.env` as in the upstream readme.
